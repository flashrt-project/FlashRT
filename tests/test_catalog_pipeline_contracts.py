...
from flash_rt.catalog import registry
from flash_rt.catalog.binding import list_bindings, load_binding
from flash_rt.catalog.registry import load as load_structure
...
def test_pipeline_catalogs_declare_shared_schedule_families():
    ...
    assert autoregressive.version == 3
    assert _stage_sets(autoregressive) == (
        {"prefill", "decode", "token_select"},
        {"input_prepare", "modality_encode", "spec_draft"},
    )
...
def test_pipeline_catalogs_do_not_own_hardware_dispatch():
    ...
    catalog_root = Path(registry.__file__).resolve().parent / "structures"
