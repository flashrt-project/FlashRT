...
PUBLIC_API_PRESET = {
    ...
    "decoder_attn_mqa": 0,
    "decoder_phase_cta": 0,
    "decoder_fused_geglu_earlyb": 0,
    "decoder_weight_evict_first": 0,
    "decoder_seq": 0,
...
def main() -> int:
    ...
    parser.add_argument("--pdl-fvk", type=int, choices=(0, 1), default=0,
                        help="with --pdl 1: also PDL-launch the rope/softmax/FP8 quantize kernels and the FP8 encoder GEMM")
    parser.add_argument(
    ...
    parser.add_argument(
        "--decoder-phase-cta", type=int, choices=(0, 1, 2), default=0,
        help="Fused MQA decoder attention (QK^T/softmax/PV + split merge + "
             "NVFP4 quantize in one kernel; replaces the cuBLAS chain)")
    parser.add_argument(
        "--decoder-weight-evict-first", type=int, choices=(0, 1), default=0,
        help="Fused MQA decoder attention (QK^T/softmax/PV + split merge + "
             "NVFP4 quantize in one kernel; replaces the cuBLAS chain)")
    parser.add_argument(
        "--decoder-fused-geglu-earlyb", type=int, choices=(0, 3, 5, 7), default=0,
        help="Fused MQA decoder attention (QK^T/softmax/PV + split merge + "
             "NVFP4 quantize in one kernel; replaces the cuBLAS chain)")
    parser.add_argument(
        "--decoder-seq", type=int, choices=(0, 1), default=0,
    ...
    if args.child_mode is not None:
        ...
        if args.construct == "load_model":
            ...
            pipe = Pi05TorchFrontendThorFP4(
                ...
                decoder_phase_cta=int(args.decoder_phase_cta),
                decoder_fused_geglu_earlyb=int(args.decoder_fused_geglu_earlyb),
                decoder_weight_evict_first=int(args.decoder_weight_evict_first),
    ...
    for mode in ("fp8", "fp4"):
        command = [
            ...
            "--decoder-phase-cta", str(args.decoder_phase_cta),
            "--decoder-fused-geglu-earlyb", str(args.decoder_fused_geglu_earlyb),
            "--decoder-weight-evict-first", str(args.decoder_weight_evict_first),
