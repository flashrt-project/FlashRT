...
| variable | default | sources | artifacts |
|---|---|---|---|
| `FLASHRT_ENABLE_NPU_PI05` | `ON` | `csrc/npu/kernels/` | the four above |
| `FLASHRT_ENABLE_NPU_GROOT_N17` | `OFF` | `csrc/npu/kernels/groot_n17/` | `libflashrt_npu_groot_n17_*.so` |

At least one has to be `ON`, and a value that is neither `ON` nor `OFF` — an
empty one included — is refused by name rather than treated as a default.
**A deselected model's libraries are removed rather than left behind**, so the
output directory always agrees with the selection that produced it; only the
standard names are touched. See
...
`FLASHRT_NPU_ATTENTION_LIBRARY`; the GR00T N1.7 group's overrides are listed on
its own page.
