...
Run (FA4 source is vendored at third_party/fa4; set the Thor arch alias):
...
~176/120/73ms, FA4 fell back to fmha — check `pip install .[thor-fa4]` and that
``flash_rt.models.lingbot.kernel_ops._get_fa4()`` returns non-None.
"""
import argparse
import time
...
def main():
    ...
    fa4 = ko._get_fa4() is not None
    print(f"[lingbot] FA4 (denoise/prefix attention) active: {fa4}"
