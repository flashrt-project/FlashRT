...
Env gates:
    FLASH_RT_MOTUS_USE_EASYCACHE=1            -> activate (default off)
    FLASH_RT_MOTUS_EASYCACHE_TARGET_SKIPS=7   -> #skip steps target
    FLASH_RT_MOTUS_EASYCACHE_MAX_RUN=8        -> cap consecutive skips
"""
...
def _apply_schedule(pipeline) -> dict:
    ...
    target = int(os.environ.get(
        "FLASH_RT_MOTUS_EASYCACHE_TARGET_SKIPS", "7"))
    max_run = int(os.environ.get(
        "FLASH_RT_MOTUS_EASYCACHE_MAX_RUN", str(num_steps)))
...
def install_motus_easycache(pipeline) -> dict:
    ...
    """
    if os.environ.get("FLASH_RT_MOTUS_USE_EASYCACHE", "0") != "1":
