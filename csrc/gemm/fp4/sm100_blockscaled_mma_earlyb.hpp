// ============================================================================
//  FlashRT fork of cutlass/gemm/collective/sm100_blockscaled_mma_warpspecialized.hpp
//  (CUTLASS v4.4.2). One change: the producer warp issues the B / SFB
//  (weight) TMA loads for the first pipeline stages BEFORE
//  griddepcontrol.wait and only the A / SFA (activation) loads after it, so
//  a programmatically-launched GEMM streams its weights while the previous
//  kernel drains. The kernel-level waits must be compiled out in the
//  translation unit that instantiates this policy (#undef
//  CUTLASS_ENABLE_GDC_FOR_SM100 before the CUTLASS includes); the wait and
//  the launch_dependents trigger live in load() instead.
// ============================================================================
#pragma once
...
struct CollectiveMma<
    MainloopSm100TmaUmmaWarpSpecializedBlockScaledEarlyB<
      Stages,
    ...
    TransformB_>
{
  ...
  using DispatchPolicy = MainloopSm100TmaUmmaWarpSpecializedBlockScaledEarlyB<
                          Stages,
                          SchedulerPipelineStageCount,
                          AccumulatorPipelineStageCount,
                          ClusterShape,
  ...
  load(
    ...
    for (int i = 0; i < pre; ++i) {
      ...
      pre_barrier[i] = mainloop_pipeline.producer_get_barrier(mainloop_pipe_producer_state);
      pre_stage[i] = mainloop_pipe_producer_state.index();
