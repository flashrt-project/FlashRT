// ============================================================================
//  FlashRT fork of cutlass/gemm/kernel/sm100_gemm_tma_warpspecialized.hpp
//  (CUTLASS v4.4.2), selected by the KernelTmaWarpSpecializedBlockScaledSm100Seq
//  schedule tag. Step 0 of the persistent decoder: identical behaviour to the
//  stock kernel (validated bit-exact), so the dependent-GEMM-sequence logic
//  (problem list, grid barrier between problems, weight prefetch across the
...
class GemmUniversal<
  ...
  TileSchedulerTag_,
  cute::enable_if_t<
    cutlass::gemm::is_seq_schedule<typename CollectiveMainloop_::DispatchPolicy::Schedule>::value>>
{
public:
  //
  // Type Aliases
