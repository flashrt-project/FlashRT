...
Three units in `csrc/npu/kernels/groot_n17/`, built only with
`FLASHRT_ENABLE_NPU_GROOT_N17=ON`, and producing
`libflashrt_npu_groot_n17_*.so`. Each serves one model's fixed geometries, so the
model owns the directory and the artifacts while the file names stay operation
named like the rest of this subtree — a second DiT path arriving later gets its
own directory rather than competing for `dit_attn`. Their Python wrappers are
under `flash_rt/npu/models/groot_n17/` for the same reason, and every one of them
refuses anything outside its envelope with a code rather than returning an
undefined result. See `docs/deployment_npu_groot_n17.md`.
